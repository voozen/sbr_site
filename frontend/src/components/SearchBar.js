...
function SearchBarLabel() {
  return (
      <div><span style = {{ fontSize: '1.5em', fontWeight: '600' }}>Znajdź
          lekarza i umów<br />wizytę<br />
      </span>
            <span style={{fontSize: '1.4em', color: '#C0C0C0', fontWeight: '600'}}>
                Wybierz spośród tysięcy<br />
          specjalistów z całej Polski</span>
        </div>);
}
...
function MainInputField({filterText, onFilterTextChange}) {
    ...
    paddingTop: '0.5em', paddingBottom: '0.5em'}}>
            <input
    type = 'text'
    value = {filterText} placeholder = 'Specjalizacja, badanie...'
                onChange={(e) => onFilterTextChange(e.target.value)}
                style={
    {
      borderRadius: '0.25em', border: '0.1em solid #666', background: '#FFF',
          width: '20em', height: '2.5em', paddingLeft: '0.5em'
    }}
            />
        </div>
    );
...
function CityInputField({filterText, onFilterTextChange}) {
    ...
    paddingBottom: '0.5em'}}>
            <input
    type = 'text'
    value = {filterText} placeholder = 'Miasto...'
                onChange={(e) => onFilterTextChange(e.target.value)}
                style={
    {
      borderRadius: '0.25em', border: '0.1em solid #666', background: '#FFF',
          width: '10em', height: '2.5em', paddingLeft: '0.5em'
    }}
            />
        </div>
    );
    ...
    if (mainFilterText && cityFilterText) {
      ...
      axios
          .get('http://localhost:8080/searchDoctor?cat=' + params.mainFilter +
               '&cit=' + params.cityFilter)
          .then(response => { setDoctors(response.data); })
          .catch(error => {
            console.error('There was a problem with your axios operation:',
                          error);
          });
    ...
    return (
        <div>
            <button
                style={
    {
      width: '5em', height: '2em', borderRadius: '0.75em',
          border: '0.1em solid #666', background: '#FFF'
    }}
                onClick={handleClick}>
                Szukaj
            </button>
