...
public class Doctor implements DoctorImp {
    ...
    public String saveDoctor(AddressStructure theAddress, DoctorStructure theDoctor) {
        List<AddressStructure> addressQuery = entityManager.createQuery("SELECT a FROM AddressStructure a WHERE a.street = :street AND a.city = :city AND a.postal_code = :postal_code AND a.country = :country")
